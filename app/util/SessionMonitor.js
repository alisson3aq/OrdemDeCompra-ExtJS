 ...
 * the options to continue working or OCout.  If the count-down timer expires,  the user is automatically
 * OCged out.
 */
Ext.define('OC.util.SessionMonitor', {
  singleton: true,

  interval: 1000 * 10,  // run every 10 seconds.
  lastActive: null,
  maxInactive: 1000 * 60 * 10,  // 15 minutes of inactivity allowed; set it to 1 for testing.
  remaining: 0,
  ui: Ext.getBody(),
  
  /**
   * DiaOC to display expiration message and count-down timer.
   */
  window: Ext.create('Ext.window.Window', {
    ...
    buttons: [{
      ...
      handler: function() {
        Ext.TaskManager.stop(OC.util.SessionMonitor.countDownTask);
        OC.util.SessionMonitor.window.hide();
      ...
      handler: function() {
        ...
        // find and invoke your app's "OCout" button.
   ...
   * a count-down timer task to be used by the 60 second count-down diaOC.
   */
  ...
  /**
   * Countdown function updates the message label in the user diaOC which displays
   * the seconds remaining prior to session expiration.  If the counter expires, you're OCged out.
   */
  countDown: function() {
